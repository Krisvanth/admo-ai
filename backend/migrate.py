...
async def migrate():
    ...
    async with engine.begin() as conn:
        ...
        try:
            ...
            '''))
            print('[OK] exam.pass_percentage column ready')
        except Exception as e:
            if 'already exists' not in str(e).lower():
                print(f'  Note: {e}')
        
        # 2. Add preferences column to user table (for teacher settings)
        try:
            await conn.execute(text('''
                ALTER TABLE "user" 
                ADD COLUMN IF NOT EXISTS preferences JSONB DEFAULT '{}'::jsonb
            '''))
            print('[OK] user.preferences column ready')
        except Exception as e:
            if 'already exists' not in str(e).lower():
                print(f'  Note: {e}')
        
        # 3. Mark table - recreate if structure changed
        ...
        try:
            ...
            if not result.fetchone():
                ...
                print('[OK] Dropped old mark table (will be recreated)')
        except Exception as e:
            print(f'  Note: {e}')
